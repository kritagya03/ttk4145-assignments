...
void main(){
    ...
    assert(val[4..8].all!("(a & 1) == 0"),
        format("Test 2You will implement the functionality for `allocate` and `deallocate`. failed: Did not run high priority (even id) users first, instead ran %s", val[4..8]));
